...
function addComponent(compId, x, y) {
  ...
  // Default pin assignments — prefer recommended pins
  const pinAssign = {};
  comp.pinGroups.forEach(pg => {
    if (pg.fixed) {
      pinAssign[pg.id] = pg.fixedPin;
    } else {
      pinAssign[pg.id] = pg.preferred || '';
    }
  });
...
function renderPanel(id) {
  ...
  if (assignable.length) {
    ...
    assignable.forEach(pg => {
      ...
      const compatPins = ESP32S3_PINS.filter(p => p.types.some(t => pg.type === t || t === 'gpio'));
      html += `<tr>
        <td>${pg.label}</td>
        <td><select data-pg="${pg.id}" onchange="onPinChange(${id},'${pg.id}',this.value)">
          <option value="">— select —</option>
          ${compatPins.map(p => {
            const taken = usedPins.includes(p.id) ? ' (in use)' : '';
            const sel   = inst.pinAssign[pg.id] === p.id ? 'selected' : '';
            return `<option value="${p.id}" ${sel} ${taken ? 'style="color:var(--yellow)"' : ''}>${p.label}${taken}</option>`;
          }).join('')}
        </select></td>
      </tr>`;
    });
...
function openPinReassign(instId, pgId, anchorEl) {
  ...
  pop.id = 'pin-popover';
  pop.style.cssText = `position:fixed;z-index:2000;background:var(--bg2);border:1px solid var(--border2);border-radius:6px;padding:8px;min-width:160px;box-shadow:0 8px 24px rgba(0,0,0,0.5)`;
  ...
  pop.innerHTML = `<div style="font-family:var(--font-mono);font-size:0.6rem;color:var(--text3);margin-bottom:6px;letter-spacing:0.1em">ASSIGN ${pg.label}</div>` +
    compatPins.map(p => {
      ...
      return `<div style="padding:5px 8px;border-radius:4px;cursor:pointer;font-family:var(--font-mono);font-size:0.72rem;
        color:${taken?'var(--yellow)':sel?'var(--green)':'var(--text)'};
...
function updateWires() {
  ...
  state.placed.forEach(inst => {
    ...
    // Component anchor point — center-left or center-right depending on chip position
    const compCx = compRect.left - cRect.left + compRect.width / 2;
    const compCy = compRect.top  - cRect.top  + compRect.height / 2;
    ...
    comp.pinGroups.forEach((pg, idx) => {
      ...
      // L-shaped routing with per-pin y-offset to reduce overlap
      const offset = (idx - comp.pinGroups.length / 2) * 10;
      const midY = compCy + offset;
      const d = routeWire(compCx, compCy + offset, pinPos.x, pinPos.y, compCx, midY);
      ...
      if (passive) {
        drawPassiveSymbol(compCx, midY, pinPos, passive);
      }
    });
  });
}

function routeWire(x1, y1, x2, y2, midX, midY) {
  // L-shaped routing: go horizontal from component, then vertical, then horizontal to pin
  const mx = (x1 + x2) / 2;
