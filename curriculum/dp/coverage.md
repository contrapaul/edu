...
*Revision pass, Aug 2026: all five sub-points rewritten for ESL readability (shorter sentences, fewer stacked clauses) and the content gaps below were closed. Remaining notes are image/graphic requests only.*

- **A2.1.1** (UCD foundation) — ✅ Comprehensive. The needs/wants/limitations framework and Schmookler citation remain, and the gaps are now filled: the Donald Norman origin story (1980s, both named books), the overcomplication examples (microwave ovens/TV remotes, ATM standardisation), the temperature/magnetic-field environment note and the texting-while-walking statistic are all present. Bonus IDEO Shopping Cart case study retained.

Need to add overcomplication example images (microwave control panel, ATM, texting-while-walking accident statistic).

- **A2.1.2** (UCD process) — ✅ Comprehensive. The five-stage process now names the disciplines that lead each stage (psychologists/ethnographers/anthropologists in Research; industrial designers/psychologists/engineers in Concept), and the Henry Petroski stay-on-tab can example anchors the Launch stage. The disadvantages list also picked up MD's "conflicting data may force earlier stages to be repeated" point.

Need to add stay-on-tab can historical image (Petroski example).

- **A2.1.3** (Multidisciplinary teams) — ✅ Comprehensive. Now lists all ~10 disciplines (adding designers/visual artists, software/hardware/systems engineers and human factors engineers), itemizes MD's five benefits of multidisciplinary teams, notes the management cost, and restores both citations (Don Norman's argument and the Chammas/Quaresma/Mont'Alvão 2015 quote). The Mercury spacesuit case study is broken into its four named innovations rather than one dense paragraph.

Need to add multidisciplinary team diagram showing all ~10 named disciplines.

- **A2.1.4** (Research methods) — ✅ Comprehensive. The qualitative/quantitative framing, comparison table and interactive matching exercise are retained, and each of the six methods now carries MD's worked business example as an "In practice" note: supermarket field research, gaming-company observation, telehealth interviews, kitchen-appliance survey, coffee-machine focus group. (Task analysis is the one method without a business example; it is covered in depth in B1.1.5 instead.)

Need to add worked business-example images (supermarket field research, gaming user observation, coffee-machine focus group).

- **A2.1.5** (Personas & stereotypes) — ✅ Comprehensive. Adds the **use case** definition (the repeat gap shared with B1.1.3), MD's 12-item persona-profile checklist, the formal 5-item list of criticisms levelled at personas, the scenario cost/assumption caveat, and the fuller population-stereotype history (UK/Australia/US switch conventions, the UK/Australia plug earth-pin comparison, and the driving-side history with its four switch dates). The Demographics "Key Concept" sidebar is unchanged.

Need to add driving-side history timeline/map and UK/Australia plug earth-pin safety comparison graphic.

**A2.1 overall:** Now comprehensive across all five sub-points. The named case studies, citations and itemized lists that had been thinned out are restored, and A2.1.3 has gone from the weakest sub-point in the topic to a complete one. Everything outstanding on this page is a graphic, not text.
...
*Revision pass, Aug 2026: all five sub-points rewritten for ESL readability, three named-concept gaps closed, and several internal inconsistencies fixed (see the note after B1.1.5).*
...
- **B1.1.2** (Research Methods) — ✅ Comprehensive. Field research, observation, interviews, questionnaires and focus groups are all covered, backed by the original "Observer Effect" deep-dive and three interactive tools (field-assignment generator, focus-group question generator, Likert survey simulator) that MD doesn't have. Gaps now closed: the **Method of Extremes** has its own Key Concept box (95th-percentile clearances, 5th-percentile control forces, the exclusion caveat, and the link onward to universal design in C1.2), the Likert scale carries its Rensis Likert 1932 origin, the focus-group facilitator's four duties are itemized, and the group size is corrected to MD's 8–12. SUS attribution (John Brooke, 1986) and the odd/even scoring alternation were added under 1.1.4, where SUS is taught.
...
- **B1.1.4** (Usability / Five E's) — ✅ Comprehensive, and corrects both the source and an earlier internal inconsistency. MD's own naming of the Five E's is inconsistent (its numbered list maps "Effective" to functionality and "Efficient" to memorability). The HTML's usability grid always used the standard Learnability/Efficiency/Memorability/Errors/Satisfaction mapping, but the "Five E's applied" activity cards below it did not: they were numbered against the alternative wording, and the bicycle card was titled "Efficient" while describing memorability. The cards are now numbered E1–E5 to match the grid, a Contactless-payment card fills the previously-missing Efficiency slot, and the fire extinguisher is kept as a clearly-marked sixth card explaining "Effective" as the extra term the IB list omits. The naming note underneath was rewritten to state honestly that the two lists are not identical. Also added: SUS attribution and its odd/even mechanic, plus four more poka-yoke examples with the Toyota/1960s origin. Still missing: MD's habit of writing each objective as a measurable benchmark (e.g., "85% of registrations complete without omission") — a useful modelling exercise for IA-style success criteria.

Need to add poka-yoke examples gallery (battery-placement keying, child-proof medication caps, colour-coded fuel nozzles, medical sponge-count checklists, limit switches).

- **B1.1.5** (Task Analysis) — ✅ Comprehensive. HTA is well covered, with the original cognitive-load concept and its three-part fix, a worked mobile-checkout example and an interactive HTA generator. **Cognitive Task Analysis (CTA)** is now named and contrasted with HTA (structure vs thinking), along with MD's point that the two are used together and that task analysis is iterative. Poka-yoke remains folded into 1.1.4, now with 7 named examples. The chain-saw safety case study is still absent, but it leans on a stripped diagram (lettered references A–G with no image) — an MD limitation rather than a pure HTML gap, though the "interlocking safety triggers" concept could still be explained in prose.

**B1.1 overall:** Strong original pedagogical value — the interactive tools (persona builder, task-analysis generator, Likert simulator, four planning scenarios) go well beyond anything in the MD and directly serve the IA-adjacent framing the page sets for itself. All three previously-dropped named concepts are now restored: the Method of Extremes (1.1.2), the use case (1.1.3, matching A2.1.5) and Cognitive Task Analysis (1.1.5). Housekeeping fixed in the same pass: an unclosed `<p>` in the section intro, two tables whose 中文 column headers had no cells (now filled), the Five E's card mismatch described above, and a duplicated poka-yoke reference entry pointing at the "Idiot-proof" article (retitled to match its actual link).
