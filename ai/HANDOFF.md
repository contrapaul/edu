...
- **What was built.** Six story screens, each with its own hue: *why this site* (the title types itself in, then the copy steps in one line at a time), *who I am*, *what I have seen* (with a `teacher-hypocrisy` voice slot, reusing page three's tag), *what I believe* (the page's one sentence assembles word by word on scroll, pinned on wide viewports like the wall screen), *how it was made* (the site's own AI record in an evidence-style `about-cite` block, in the very format page three's builder teaches), and *the end* (a thanks line types, a "Read in full" stamp lands, the closing words arrive; it plays once, when the section is half on screen, and appears instantly for a reader who already earned it).
- **The read-through.** The rail is the page's read receipt: each part's dot fills as that part scrolls past the top half of the viewport (IntersectionObserver plus a scroll sweep, and a one-time sweep on load for mid-page refreshes). The last part is read when the end moment plays. The marks go into the shared `ai-progress` store under about's own ids (`why`, `who`, `seen`, `believe`, `made`, `end`), so page one's recap, which tallies only `s*` ids, is untouched. Page one's "Clear my progress" button wipes them with the rest, which is honest behaviour for a clear-all.
- **Placeholder copy.** Every Paul slot is bracketed `[Placeholder: ...]`: identity details, the two prose beats in *what I have seen*, the position under the assembling sentence, the tool/date/what-it-did/what-I-did rows in the site's own record, the closing words, and the credits line. The one-sentence manifesto ("You will be using these tools, whether or not it feels allowed. This site is about what using them well looks like.") is a stand-in for his position, to be rewritten with it. The end band loops back to page one.
- **Shared parts.** `typed.js` moved from `js/index/` up to `js/` (it is a utility, and about needs it; the two importers in `js/index/` updated to match). New files: `about.html`, `css/about.css` (typed cursor, after-typed, assemble stage, about-cite, end moment; every rule has a still fallback under reduced motion, and nothing is hidden without JS), `js/about/main.js`. The nav's About link is live on all four pages (the `is-soon` is gone; about marks itself current). The glossary test's page list gained `about.html`. 46/46.
- **The meta moment.** *How it was made* is the site practicing its own citation advice: the tool named, the date stated, the split between model and Paul explicit, and a "What to check" row pointing at page one's two verbatim transcripts as the checkable core. This is the page the brief's about/rationale item was waiting for.
...
5. Page two: `learn.html` is still four `Placeholder` panels (conversation, sources, memory, organising); the activities are specified in `interactive.md`.
6. **The About page is waiting on Paul.** `about.html` is built with the structure and all the motion; every factual slot is a bracketed `[Placeholder: ...]` (identity, the *what I have seen* beats, the position under the assembling sentence, the tool/date/split rows in the site's own record, the closing words, the credits). The placeholder manifesto sentence and the `[school] · [year] · [contact]` credits are stand-ins. No further build needed until he writes.
7. If page one's copy is rewritten, `data/tokens-page1.json` must be regenerated, and the script that does it lives only in session history, not in the repo; get it into `dev/` before the rewrite.
8. The "Compare with a class" stub on page three waits on the live-session backend (the blurt/cloud pattern), built last.
